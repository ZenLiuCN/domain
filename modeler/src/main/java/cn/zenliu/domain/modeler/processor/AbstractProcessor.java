...
public abstract class AbstractProcessor {
    ...
    protected void info(String pattern, Object... args) {
        System.out.println(MessageFormatter.arrayFormat("[" + name() + "\t] [INFO] " + pattern, args).getMessage());
    }
    ...
    protected void errorf(String pattern, Object... args) {
        var m = MessageFormatter.arrayFormat("[" + name() + "\t] [ERROR] " + pattern, args);
    ...
    protected void fatalf(String pattern, Object... args) {
        var m = MessageFormatter.arrayFormat("[" + name() + "\t] [FATAL]" + pattern, args);
        System.err.println(m.getMessage());
        if (m.getThrowable() != null) {
            m.getThrowable().printStackTrace(System.err);
     ...
     */
    protected @org.jetbrains.annotations.Nullable Configurer.Config preCheck(Element ele, ProcUtil u) {
        if (u.isGeneratedBy(ele, name())) {
            return null;
        }
        var c = Configurer.resolve(u, ele);
        if (!(c.enabled(self().getPrefix()))) {
            if (c.debug()) u.mandatoryWarn(self(), "disabled when process {}", ele);
            return null;
        }
        if (c.debug()) debug("Configs {} for target '{}'", c, ele);
        return c;
    }
